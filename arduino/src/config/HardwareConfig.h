...
// Pi 4 USB serial is the active control source. Hardware adapters remain fully
// active so this sketch exercises the same pins and enable/disable behavior as
// the assembled robot even when the shield, LEDs, and bumpers are disconnected.
constexpr bool kSerialControlEnabled = true;
constexpr bool kDriveHardwareEnabled = true;
constexpr bool kLedHardwareEnabled = true;
constexpr bool kBumperHardwareEnabled = true;
constexpr bool kLocalJoystickEnabled = false;
constexpr bool kRequireNeutralRearm = true;
...
constexpr bool kBenchTelemetryEnabled = false;
// Minimum time between event-driven telemetry records.
constexpr unsigned long kBenchTelemetryIntervalMs = 500;

// Pi-to-Mega protocol and safety timing. Serial frames arrive at 25 Hz. The
// independent timeouts stop accepting motion if either the USB heartbeat or
// the underlying Pico sample becomes stale.
constexpr uint8_t kSerialProtocolVersion = 2;
constexpr unsigned long kSerialCommandTimeoutMs = 200;
constexpr unsigned long kRemoteCommandTimeoutMs = 300;
constexpr unsigned long kSerialTelemetryIntervalMs = 100;
constexpr uint8_t kSerialMaximumBytesPerLoop = 24;
constexpr uint8_t kSerialMaximumLineLength = 72;
