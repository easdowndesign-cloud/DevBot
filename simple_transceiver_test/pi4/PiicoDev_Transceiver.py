...
class PiicoDev_Transceiver(object):
        ...
        for _ in range(3):
            try:
                ...
                if data is None or len(data) != length:
                    raise OSError("incomplete I2C read")
                self._last_i2c_error = None
                return data
            except Exception as error:
                last_error = error
                sleep_ms(3)
        self._last_i2c_error = last_error
        return None
    ...
    def _wait_until_ready(self, attempts=200):
        for _ in range(attempts):
            if self._read_int(_REG_TRANSCEIVER_READY) == 1:
                return
            sleep_ms(10)
        detail = repr(getattr(self, '_last_i2c_error', None))
        raise RuntimeError(
            "Transceiver at I2C address 0x{:02X} did not become ready; last I2C error: {}".format(
                self.address, detail
            )
        )
    ...
    def _receive_payload(self):
        ...
        if self._payload_new == 1:
            if self.debug:
                ...
                print('delay')
            received_length = self._read_int(_REG_PAYLOAD_LENGTH)
            if received_length is None or received_length > _MAXIMUM_PAYLOAD_LENGTH:
                return 0, bytes(0)
            payload_length = received_length + 3 # _MAXIMUM_PAYLOAD_LENGTH + RSSI + source_radio_address
            ...
            for i in range(number_of_chunks):
                ...
                if chunk_length > 0:
                    chunk = self._read(_REG_PAYLOAD, length=chunk_length)
                    if chunk is None:
                        return 0, bytes(0)
    ...
    def tx_power(self):
        ...
        self._wait_until_ready()
        data = self._read(_REG_TX_POWER)
        if data is None:
            raise RuntimeError("Unable to read transceiver TX power")
        value = unpack('b', bytes(data))
        return value[0]
